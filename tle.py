...
def find_next_santa_barbara_overfly(satellite, start_time, max_hours=48):
    """Find the next time the satellite overflies Santa Barbara, CA"""
    # Santa Barbara coordinates
    sb_lat = 34.4208  # Santa Barbara latitude
    sb_lon = -119.6982  # Santa Barbara longitude
    ...
    for minute in range(0, max_hours * 60, 5):  # Check every 5 minutes
        ...
        if error_code == 0:
            ...
            distance = calculate_distance(sat_lat, sat_lon, sb_lat, sb_lon)
...
if error_code == 0:
    ...
    # Find next Santa Barbara overfly
    print("\n" + "="*50)
    print("SANTA BARBARA OVERFLY CALCULATION")
    print("="*50)
    next_overfly_time, closest_distance = find_next_santa_barbara_overfly(satellite, now)
    ...
    if next_overfly_time:
        ...
        print(f"Next Santa Barbara overfly:")
        ...
        print("No Santa Barbara overfly found in the next 48 hours")
    print("="*50 + "\n")
    ...
    ax.set_title('Satellite Ground Track (Previous 90 min + Next 90 min)', color='#6bb86b', fontsize=14, fontweight='bold')
